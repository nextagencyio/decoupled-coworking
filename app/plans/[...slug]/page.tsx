...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/plans/${slug.join('/')}`
  const item = await getPlan(path)

  if (!item) {
    return { title: 'Plan Not Found | Coworking' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).priceMonthly && (
                    <div>
                      <dt className="text-sm text-gray-500">Monthly Price</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).priceMonthly}</dd>
                    </div>
                  )}
                  {(item as any).idealFor && (
                    <div>
                      <dt className="text-sm text-gray-500">Ideal For</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).idealFor}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
