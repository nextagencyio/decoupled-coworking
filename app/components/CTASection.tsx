...
export default function CTASection({ homepageContent }: CTASectionProps) {
  const title = (homepageContent as any)?.ctaTitle || 'Ready to Join the Community?'
  const description = (homepageContent as any)?.ctaDescription?.processed || ''
  const primaryLabel = (homepageContent as any)?.ctaPrimary || 'Book a Tour'
  const secondaryLabel = (homepageContent as any)?.ctaSecondary || 'View Plans'
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-8">
        ...
        {description ? (
          ...
          <p className="text-primary-100 text-lg mb-10 max-w-2xl mx-auto">
            Start with a free day pass and discover why hundreds of professionals call The Nexus Hub home.
          </p>
        )}
        <div className="flex justify-center gap-4 flex-wrap">
          <Link
            href="/contact"
            className="px-10 py-4 bg-accent-500 text-white rounded-lg hover:bg-accent-400 transition-all duration-200 font-display font-bold text-lg uppercase tracking-wider shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
            {primaryLabel}
          </Link>
          <Link
