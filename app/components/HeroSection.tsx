...
export default function HeroSection({ homepageContent }: HeroSectionProps) {
  const title = (homepageContent as any)?.heroTitle || (homepageContent as any)?.title || 'WHERE IDEAS CONNECT'
  const subtitle = (homepageContent as any)?.heroSubtitle || 'Premium coworking for freelancers, startups, and remote teams.'
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-0 left-0 right-0 h-2 bg-accent-500"
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28 md:py-40 lg:py-48 text-center">
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          <Link
            href="/plans"
            className="px-8 py-4 bg-accent-500 text-white rounded-lg hover:bg-accent-400 transition-all duration-200 font-display font-bold text-lg uppercase tracking-wider shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
            View Plans
          </Link>
          <Link
