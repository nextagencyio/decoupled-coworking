...
export const metadata: Metadata = {
  ...
  description: 'Discover the premium amenities at The Nexus Hub coworking space.',
}

async function getAmenities() {
  try {
    const client = getClient()
    const { data } = await client.raw(GET_AMENITIES, { first: 50 })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="text-center py-12">
              <h2 className="text-2xl font-display font-semibold text-gray-600 mb-2 uppercase tracking-wide">No Amenities Yet</h2>
              <p className="text-gray-500">Amenities will appear here once content is imported.</p>
            </div>
          ) : (
